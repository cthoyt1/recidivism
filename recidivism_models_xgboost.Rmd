...
setwd("~/Learning/STAT-656/recidivism")
set.seed(1)
...
```{r}
outLogisticTrans = train(x = XtrainLogTrans, 
                         y = Ytrain, method = 'xgboost',
                         trControl = trControl)
```

```{r}
pcaOut = prcomp(XtrainLogTrans,center=TRUE,scale=TRUE)

pca_full <- data.frame(cbind(Ytrain,pcaOut$x[,1:2], XtrainLogTrans)) %>% select(-V21, -V22, -V23, -V23, -V24, -V25)

names(pca_full)

write.csv(pca_full, 'shinyapp/pca_data.csv', row.names=FALSE)

pca_plot <- plot(pcaOut$x[,1:2], asp = 1)
```
...
Xtrain[c(94,118,1447,1795,2708,3368),]
...
boostOut   = train(x = XtrainLogTrans, y = Ytrain,
                   method = "xgbTree", verbose = 0,
                   tuneGrid = tuneGrid,
                   trControl = trControl)
plot(boostOut)

boostImportance0 = xgboost::xgb.importance(model = boostOut$finalModel) 
xgboost_import <- boostImportance0[,1:2] %>% arrange(desc(Gain))

ggplot(data=xgboost_import, aes(x=reorder(Feature, Gain), y=Gain)) +
  geom_bar(stat='identity',
           fill = 'dark green') + 
  coord_flip() +
  xlab("Feature") +
  ggtitle("XGBTree Feature Importance") +
...
rpartOut = train(x = XtrainLogTrans, y = Ytrain,
                  method = "rpart",
                  tuneGrid = tuneGrid,
                  trControl = trControl)
